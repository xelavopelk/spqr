...
func TestMultiShardRouting(t *testing.T) {
	...
	distribution := "ds1"
	stmts, err := db.CreateDistribution(context.TODO(), &qdb.Distribution{
		ID:       distribution,
	...
	assert.NoError(err)
	err = db.ExecNoTransaction(context.TODO(), stmts)
	assert.NoError(err)
...
func TestScatterQueryRoutingEngineV2(t *testing.T) {
	...
	stmts, err = db.CreateKeyRange(context.TODO(), (&kr.KeyRange{
		ShardID:      "sh1",
...
func TestSetStmt(t *testing.T) {
	...
	stmts, err := db.CreateDistribution(context.TODO(), qdb.NewDistribution(distribution1, nil))
	assert.NoError(err)
	err = db.ExecNoTransaction(context.TODO(), stmts)
	assert.NoError(err)

	stmts, err = db.CreateDistribution(context.TODO(), qdb.NewDistribution(distribution2, nil))
	assert.NoError(err)
	err = db.ExecNoTransaction(context.TODO(), stmts)
	assert.NoError(err)

	stmts, err = db.CreateKeyRange(context.TODO(), &qdb.KeyRange{
		ShardID:        "sh1",
...
func TestRouteWithRules_Select(t *testing.T) {
	...
	stmts, err := db.CreateDistribution(context.TODO(), distribution)
	assert.NoError(err)
	err = db.ExecNoTransaction(context.TODO(), stmts)
	assert.NoError(err)

	stmts, err = db.CreateDistribution(context.TODO(), unusedDistribution)
	assert.NoError(err)
	err = db.ExecNoTransaction(context.TODO(), stmts)
	assert.NoError(err)
...
func TestHashRouting(t *testing.T) {
	...
	db, _ := qdb.NewMemQDB(MemQDBPath)
	distribution1 := "ds1"
	stmts, err := db.CreateDistribution(context.TODO(),
